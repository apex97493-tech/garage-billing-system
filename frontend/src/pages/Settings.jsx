  ...
  const handleExportBackup = async () => {
    try {
      ...
      downloadAnchor.setAttribute(
        "download",
        `workshop_backup_${new Date().toISOString().slice(0, 10)}.json`,
      );
    ...
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs">
        <h1 className="text-xl font-black text-slate-950 tracking-tight">
          Workshop Profile & Automation Settings
        </h1>
        <p className="text-xs text-slate-500 mt-0.5 font-medium">
          Customize your workshop name, logo, phone number, automated WhatsApp,
          and billing setup
        </p>
      </div>
          ...
          >
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div className="space-y-2 text-center sm:text-left flex-1">
                  <p className="text-xs font-bold text-slate-900">
                    Upload Shop Logo / Seal
                  </p>
                  <p className="text-[11px] text-slate-500 font-medium leading-relaxed">
                    Recommended: PNG or JPG with transparent or white
                    background. Appears automatically on all printed bills and
                    WhatsApp PDF invoices.
                  </p>
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSettings({
                        ...settings,
                        taxLabel: e.target.value.toUpperCase(),
                      })
                    }
                    placeholder="GST"
                  />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase mb-1">
                    UPI ID (For Instant QR on Bills)
                  </label>
                  <input
                    type="text"
                    className="w-full px-3 py-2 bg-white text-slate-900 font-medium placeholder:text-slate-400 border border-slate-300 rounded-lg text-xs focus:ring-2 focus:ring-slate-900 focus:outline-none"
              ...
              <label className="flex items-center space-x-2.5 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      autoSendBillWhatsapp: e.target.checked,
                    })
                  }
                ...
                <div>
                  <span className="text-xs font-bold text-slate-900 block">
                    Auto-Send Bill on POS Checkout
                  </span>
                  <span className="text-[11px] text-slate-600 font-medium">
                    Dispatches digital tax invoice breakdown to customer's
                    WhatsApp instantly upon saving bill
                  </span>
                </div>
              </label>
              ...
              <label className="flex items-center space-x-2.5 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      autoSendServiceReminders: e.target.checked,
                    })
                  }
                  className="rounded text-slate-900 focus:ring-0 w-4 h-4"
                />
                <div>
                  <span className="text-xs font-bold text-slate-900 block">
                    Daily Automated Periodic Service Reminders
                  </span>
                  <span className="text-[11px] text-slate-600 font-medium">
                    Daily 10:00 AM background check to notify customers whose
                    motorcycle is due for periodic maintenance
                  </span>
                </div>
              </label>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-3">
            ...
            <p className="text-xs text-slate-600 leading-relaxed font-medium">
              Connect your workshop's WhatsApp number to automatically dispatch
              PDF bills and periodic service reminders to your customers.
            </p>
            ...
            {whatsappStatus.isConnected ? (
              <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl space-y-2.5 text-xs">
                <div className="flex items-center text-emerald-900 font-bold">
                  ...
                  <span>
                    Linked Phone:{" "}
                    {formatDisplayPhone(whatsappStatus.connectedPhone) ||
                      "Workshop Device"}
                  </span>
                </div>
                <p className="text-[11px] text-emerald-800 font-medium leading-normal">
                  Automatic invoice document delivery and service maintenance
                  ...
                  <div className="text-center p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                    ...
                    <div className="flex items-center justify-center pt-1">
                      <button
                        ...
                        className="text-[11px] font-bold text-slate-600 hover:text-slate-900 flex items-center transition-colors"
                      >
            ...
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 text-xs space-y-1">
              <div className="flex justify-between font-medium text-slate-600">
                ...
                <strong className="text-slate-900 font-mono">
                  {dbStats.parts.toLocaleString()}
                </strong>
              </div>
              <div className="flex justify-between font-medium text-slate-600">
                <span>Customer Records:</span>
                <strong className="text-slate-900 font-mono">
                  {dbStats.customers.toLocaleString()}
                </strong>
              </div>
              <div className="flex justify-between font-medium text-slate-600">
                <span>Invoices Generated:</span>
                <strong className="text-slate-900 font-mono">
                  {dbStats.invoices.toLocaleString()}
                </strong>
              </div>
            </div>
